...
async function onSubmit() {
  ...
  try {
    ...
    const res = await customersStore.createCustomer(payload, { uuid: payload.client_uuid })
    if (res.status === 'ok' && res.id) {
      uni.showToast({ title: '创建成功', icon: 'success' })
      setTimeout(() => {
        uni.redirectTo({ url: `/pages/customer/detail?id=${res.id}` })
      }, 600)
    } else {
      // 离线 queued：返回上一页，已在 store 内 toast
      setTimeout(() => uni.navigateBack(), 600)
    }
  } catch (e) { /* http 已 toast */ } finally {
