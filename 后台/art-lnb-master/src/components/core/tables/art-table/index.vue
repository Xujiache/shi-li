...
<script setup lang="ts">
  ...
  /**
   * 规范化列宽：
   * 解决“所有列都写了 width 导致表格右侧留白，看起来不对齐”的问题。
   * 当没有任何可伸缩列时，自动把最宽的一列从 width 转为 minWidth（变为可伸缩列），让表格填满容器宽度。
   */
  function normalizeColumns(cols: ColumnOption[]): ColumnOption[] {
    const list = Array.isArray(cols) ? cols : []
    const out = list.map((c) => ({ ...c }))

    const isCandidate = (c: ColumnOption) => {
      if (!c) return false
      // 排除特殊列与固定列
      if (c.type) return false
      if (c.fixed) return false
      // 排除操作列（通常固定在右侧）
      if (c.prop === 'operation') return false
      return true
    }

    const candidates = out.filter(isCandidate)
    if (candidates.length === 0) return out

    const hasFlexible = candidates.some((c) => c.width === undefined || c.width === null || c.width === '')
    if (hasFlexible) return out

    const toNumber = (w: unknown): number | null => {
    ...
    // 选择最宽的一列改为可伸缩列，避免把“短字段”拉得过宽
    let targetIdx = -1
    let maxW = -1
    candidates.forEach((c) => {
      const n = toNumber(c.width)
      if (n != null && n > maxW) {
        maxW = n
        targetIdx = out.indexOf(c)
      }
    })
    if (targetIdx === -1) {
      // 兜底：取最后一个候选列
      const last = candidates[candidates.length - 1]
      targetIdx = out.indexOf(last)
    }
    if (targetIdx === -1) return out

    const target = out[targetIdx]
    if (target && target.width !== undefined && target.width !== null && target.width !== '') {
      target.minWidth = target.minWidth ?? target.width
      delete target.width
    }
  ...
  const renderColumns = computed(() => normalizeColumns(props.columns))
