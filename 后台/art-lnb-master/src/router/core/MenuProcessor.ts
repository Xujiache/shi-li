...
export class MenuProcessor {
  ...
  async getMenuList(): Promise<AppRouteRecord[]> {
    ...
    // CloudBase 直连模式下强制使用前端菜单：
    // - 后端菜单接口依赖传统后端鉴权（/api/v3/system/menus），会导致 401 并影响登录后的路由初始化
    // - 视力后台的菜单本身由前端路由维护
    const rawCloudbaseUrl = import.meta.env.VITE_CLOUDBASE_HTTP_URL
    const isCloudbaseEnabled = typeof rawCloudbaseUrl === 'string' && rawCloudbaseUrl.trim().length > 0

    // 仅当明确配置为 backend 且未启用 CloudBase 时才请求后端菜单
    const useBackendMenu = !isCloudbaseEnabled && isBackendMode.value === true
