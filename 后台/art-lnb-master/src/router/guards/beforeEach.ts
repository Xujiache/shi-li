...
async function handleDynamicRoutes(
  ...
  try {
    ...
    // token 失效错误（云函数返回 token无效/缺少token/token已过期）：
    // 清除登录状态，跳转到登录页重新登录
    if (isTokenError(error)) {
      console.warn('[RouteGuard] token 已失效，自动跳转到登录页')
      routeInitInProgress = false
      const userStore = useUserStore()
      userStore.logOut()
      next(false)
...
/**
 * 判断是否为 token 失效相关错误（云函数返回 token无效/缺少token/token已过期）
 * 这些错误表示需要重新登录，而非服务端异常
 */
function isTokenError(error: unknown): boolean {
  if (error instanceof Error) {
    const msg = error.message
    return msg.includes('token无效') || msg.includes('缺少token') || msg.includes('token已过期')
  }
  return false
}
